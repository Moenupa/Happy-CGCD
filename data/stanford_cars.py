...
def subsample_classes(dataset, include_classes=range(160)):
    include_classes_cars = (
        np.array(include_classes) + 1
...
def subDataset_wholeDataset(datalist):
    ...
    wholeDataset.data = np.concatenate([d.data for d in datalist], axis=0)
    wholeDataset.target = np.concatenate([d.target for d in datalist], axis=0).tolist()
...
def get_scars_datasets(
    ...
    old_dataset_all = subsample_classes(
        deepcopy(whole_training_set), include_classes=train_classes
    )  # 40000

    each_old_all_samples = [
        subsample_classes(deepcopy(old_dataset_all), include_classes=[targets])
        for targets in list(train_classes)
    ]  # 80*500   # NOTE!!!

    each_old_labeled_slices = [
        subsample_instances(samples, prop_indices_to_subsample=prop_train_labels)
    ...
    each_old_unlabeled_slices = [
        np.array(
            list(
                set(list(range(len(samples.target)))) - set(each_old_labeled_slices[i])
            )
        )
    ...
    each_old_labeled_samples = [
        subsample_dataset(deepcopy(samples), each_old_labeled_slices[i])
        for i, samples in enumerate(each_old_all_samples)
    ]  # 80*400

    each_old_unlabeled_samples = [
        subsample_dataset(deepcopy(samples), each_old_unlabeled_slices[i])
        for i, samples in enumerate(each_old_all_samples)
    ]  # 80*100 for online old classes unlabeled
    ...
    offline_test_dataset = subsample_classes(
        deepcopy(test_dataset), include_classes=list(train_classes)
    )
    ...
    for s in range(continual_session_num):
        ...
        online_session_each_old_slices = [
            np.random.choice(
                np.array(list(range(len(samples.target)))),
                online_old_seen_num,
                replace=False,
            )
    ...
    novel_unlabelled_indices = set(whole_training_set.uq_idxs) - set(
        old_dataset_all.uq_idxs
    )  # 10000
    novel_dataset_unlabelled = subsample_dataset(
        deepcopy(whole_training_set), np.array(list(novel_unlabelled_indices))
    )  # 10000

    # novel_targets_shuffle = np.array(list(set(np.array(novel_dataset_unlabelled.target).tolist())))
    novel_targets_shuffle = (
        np.array(list(set(np.array(novel_dataset_unlabelled.target).tolist()))) - 1
    )  # NOTE!!! -1
    ...
    for s in range(continual_session_num):
        online_session_targets = novel_targets_shuffle[
            0 : s * targets_per_session + targets_per_session
        ]
        if s == continual_session_num - 1:  # NOTE!!! for scars, 98//5
            online_session_targets = novel_targets_shuffle
        online_session_each_novel_samples = [
            subsample_classes(
                deepcopy(novel_dataset_unlabelled), include_classes=[targets]
            )
        ...
        for i in range(len(online_session_targets)):
            if (s >= 1) and (i < s * targets_per_session):
                online_session_each_novel_slices.append(
                    np.random.choice(
                        np.array(
                            list(
                                range(len(online_session_each_novel_samples[i].target))
                            )
                        ),
                ...
                if (
                    len(online_session_each_novel_samples[i].target)
                    > online_novel_unseen_num
                ):  # NOTE!!! for long-tailed ImageNet!!!
                    online_session_each_novel_slices.append(
                        np.random.choice(
                            np.array(
                                list(
        ...
        online_session_novel_samples = [
            subsample_dataset(deepcopy(samples), online_session_each_novel_slices[i])
            for i, samples in enumerate(online_session_each_novel_samples)
        ]  # [50, 50, ...],  len=n

        online_session_novel_dataset = subDataset_wholeDataset(
            online_session_novel_samples
        )
