...
def subDataset_wholeDataset(datalist):
    ...
    wholeDataset.targets = np.concatenate(
        [d.targets for d in datalist], axis=0
    ).tolist()
...
def get_imagenet_100_datasets(
    ...
    each_old_unlabeled_slices = [
        np.array(
            list(
                set(list(range(len(samples.targets)))) - set(each_old_labeled_slices[i])
            )
        )
    ...
    novel_targets_shuffle = np.array(
        list(set(np.array(novel_dataset_unlabelled.targets).tolist()))
    )
            ...
            if (s >= 1) and (i < s * targets_per_session):
                online_session_each_novel_slices.append(
                    np.random.choice(
                        np.array(
                            list(
                                range(len(online_session_each_novel_samples[i].targets))
                            )
                        ),
                ...
                if (
                    len(online_session_each_novel_samples[i].targets)
                        ...
                        np.random.choice(
                            np.array(
                                list(
                                    range(
                                        len(
                                            online_session_each_novel_samples[i].targets
                                        )
                                    )
                                )
                            ),
