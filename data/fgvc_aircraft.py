...
def get_aircraft_datasets(
    ...
    each_old_unlabeled_slices = [
        np.array(
            list(
                set(list(range(len(samples.samples)))) - set(each_old_labeled_slices[i])
            )
        )
    ...
    for s in range(continual_session_num):
        ...
        online_session_each_old_slices = [
            np.random.choice(
                np.array(list(range(len(samples.samples)))),
                online_old_seen_num,
                replace=False,
            )
    ...
    # NOTE!!!
    novel_cls_labels = [
        t for i, (p, t) in enumerate(novel_dataset_unlabelled.samples)
    ]  # NOTE!!!
            ...
            if (s >= 1) and (i < s * targets_per_session):
                online_session_each_novel_slices.append(
                    np.random.choice(
                        np.array(
                            list(
                                range(len(online_session_each_novel_samples[i].samples))
                            )
                        ),
