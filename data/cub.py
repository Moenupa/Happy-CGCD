...
def subDataset_wholeDataset(datalist):
    ...
    wholeDataset.data = pd.concat([d.data for d in datalist], axis=0)
...
def get_cub_datasets(
    ...
    whole_training_set = CustomCub2011(
        root=cub_root, transform=train_transform, train=True, download=False
    )
    ...
    each_old_unlabeled_slices = [
        np.array(
            list(set(list(range(len(samples.data)))) - set(each_old_labeled_slices[i]))
        )
    ...
    for s in range(continual_session_num):
        ...
        online_session_each_old_slices = [
            np.random.choice(
                np.array(list(range(len(samples.data)))),
                online_old_seen_num,
                replace=False,
            )
    ...
    novel_cls_labels = [
        int(r["target"]) - 1
        for x, (_, r) in enumerate(novel_dataset_unlabelled.data.iterrows())
    ]
        ...
        for i in range(len(online_session_targets)):
            if (s >= 1) and (i < s * targets_per_session):
                online_session_each_novel_slices.append(
                    np.random.choice(
                        np.array(
                            list(range(len(online_session_each_novel_samples[i].data)))
                        ),
