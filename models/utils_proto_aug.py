...
class ProtoAugManager:
    def __init__(
        self, feature_dim, batch_size, hardness_temp, radius_scale, device, logger
    ):
    ...
    def compute_proto_aug_loss(self, model):
        ...
        prototypes_labels = torch.randint(0, len(prototypes), (self.batch_size,)).to(
            self.device
        )  # dtype=torch.long
        prototypes_sampled = prototypes[prototypes_labels]
        prototypes_augmented = (
            prototypes_sampled
            + torch.randn((self.batch_size, self.feature_dim), device=self.device)
            * self.radius
            * self.radius_scale
        )
        ...
        proto_aug_loss = nn.CrossEntropyLoss()(
            prototypes_output / 0.1, prototypes_labels
        )
    ...
    def compute_proto_aug_hardness_aware_loss(self, model):
        ...
        prototypes_labels = np.random.choice(
            len(prototypes), size=(self.batch_size,), replace=True, p=sampling_prob
        )
    ...
    def update_prototypes_online(
        self, model, train_loader, num_seen_classes, num_all_classes
    ):
        model.eval()
        ...
        for c in range(num_seen_classes, num_all_classes):
            ...
            if len(feats_c) == 0:
                self.logger.info(
                    "No pred of this class, using fc (last_layer) parameters..."
                )
